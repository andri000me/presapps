		...
		if($cek > 0){
			if($password_baru == $kpassword_baru){
				...
				if(!$result2){
					...
					header("location:../staff/index.php?page=profil_saya&status=1");
				}
				mysqli_close($connect);
			}else{
				header("location:../staff/index.php?page=profil_saya&action=ganti_password&status=0");
			}
		}else{
			header("location:../staff/index.php?page=profil_saya&action=ganti_password&status=1");
		}
		break;
}
